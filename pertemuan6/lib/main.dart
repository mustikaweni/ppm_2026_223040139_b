...
class Catatan {
  ...
  static Catatan fromJson(Map<String, dynamic> m) {
    return Catatan(
      id: m['id'] as int?,
      judul: (m['judul'] as String?) ?? '',
      isi: (m['isi'] as String?) ?? '',
      kategori: (m['kategori'] as String?) ?? 'Umum',
      dibuatPada: m['dibuat_pada'] != null
          ? DateTime.parse(m['dibuat_pada'] as String).toLocal()
          : DateTime.now(),
    );
  }
...
class _FormPageState extends State<FormPage> {
  ...
  // DI SINI LETAK PERBAIKANNYA: Method dispose dipasang tepat di dalam class form state
  @override
  void dispose() {
    _judulCtrl.dispose();
    _isiCtrl.dispose();
